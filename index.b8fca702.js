...
startButton.addEventListener("click", (e)=>{
    if (e.target.textContent === RESTART_BUTTON_TEXT) game.restart();
    else {
        game.start();
...
document.addEventListener("keydown", (e)=>{
    if (game.getStatus() === Game.STATUS_IDLE) game.setStatus(Game.STATUS_PLAYING);
    if (game.getStatus() !== Game.STATUS_PLAYING) return;
    e.preventDefault();
    switch(e.key){
        case "ArrowUp":
            game.moveUp();
 ...
 */ class Game {
   ...
   */ checkForWin() {
        for(let i = 0; i < this.state.length; i++){
            for(let j = 0; j < this.state[i].length; j++)if (this.state[i][j] === Game.WIN_NUMBER) {
                this.gameWon();
                break;
            }
        }
